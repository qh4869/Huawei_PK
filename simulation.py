...
def simulate_on_day(player_0, player_1, day_requests, day):
    ...
    # ---输出日志---
    path = 'priceLog.txt'
    if day == 0 and os.path.exists(path) is True:
        os.remove(path)

    with open(path, 'a') as f:
        f.write('day----{0}---player_0 vs player_1\n'.format(day))
        for i in range(len(prices_0)):
            f.write('{0} \t vs \t {1} \n'.format(prices_0[i], prices_1[i]))
...
def main(argv):
    err = open('logfile.txt', 'w')

    player_0, player_1 = create_players(argv, err)
